...
import { User, AuthProviders, AuthOptions } from './auth.types';
...
export class AuthService {
  ...
  authenticate({ isSignIn, provider, user }: AuthOptions): Promise<auth.UserCredential> {
    ...
    if (provider == AuthProviders.Facebook) {
      operation = this.signUpWithPopUp(provider);
    } else {
  ...
  private signUpWithPopUp(provider: AuthProviders): Promise<auth.UserCredential> {
